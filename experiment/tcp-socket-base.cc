...
double
TcpSocketBase::Random (void)
{
  Ptr<UniformRandomVariable> x = CreateObject<UniformRandomVariable>();
  x->SetAttribute("Min",DoubleValue(1));
  x->SetAttribute("Max",DoubleValue(1.2));
...
TcpSocketBase::Send (Ptr<Packet> p, uint32_t flags)
{
  ...
  ////////////////////////////////////
  double value = Random();
  //NS_LOG_UNCOND("The random is"<<  value <<"  I send a message!\n");
  ns3::TcpSocketBase::SetMinRto(Time(Seconds(value)));
  //ns3::TcpSocketBase::SetMinRto(Time(Seconds(1.0)));    //comment this line when doing the RANDOMLIZE
                                                         //EXPERIMENT
