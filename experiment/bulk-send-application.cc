  ...
  if (!m_socket)
    {
      ...
      m_socket->TraceConnectWithoutContext("CongestionWindow",MakeCallback
		      (&BulkSendApplication::CwndChange,this));
    }
  if (m_connected)
    {
      SendData ();
...
{
  NS_LOG_FUNCTION (this);
// NS_LOG_UNCOND(Simulator::Now().GetSeconds() <<"\tI try to send!");
   if (m_connected)
